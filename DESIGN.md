...
- A right inspector dock with two tabs. The Scene tab lists every live entity and lets you select one by clicking it. The Inspector tab shows the selected entity's position and velocity as draggable number fields, a colour picker for its colour, a draggable field for its scale, a checkbox for whether it is `Controlled` (WASD-driven), and a searchable picker that assigns one of the library's scripts to the entity. Every edit writes straight back into the world.
