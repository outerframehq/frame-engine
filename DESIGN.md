...
Scripting surface and lifecycle. The variable surface a script sees is flat and hand-named (`px`, `sx`, `cr`, and so on). That is easy to expose but not very discoverable; a structured, registered type (`self.pos.x`) is the natural next step, deferred until the flat naming actually hurts. Lifecycle has rough edges too: deleting a library script leaves any entity that referenced it with a dangling name, which the runtime skips safely and the inspector flags, but renaming is not yet supported and would orphan references. And determinism under lockstep, one of the reasons `rhai` was chosen, is not yet validated across machines.

Collision is detection-only and coarse. The `collision` system finds overlapping boxes and records the pairs; scripts can now read whether their own entity is in one (a read-only `hit` flag), but nothing responds automatically — no separation, no bounce. The boxes are axis-aligned scale-boxes, not fitted to the actual mesh (a flat plane still has a tall box), and the sweep is O(n^2). An automatic response, richer script queries (which entity was hit, a contact point), mesh-fitted or rotated boxes, and a broad phase for large entity counts are all open, in roughly that order of likely usefulness.
...
Collision detection (`systems/`). A `collision` system finds which entity boxes overlap and records the pairs on the world (`World.collisions`). It is detection only — a *trigger*: it never separates entities, changes a velocity, or otherwise responds. Each box is axis-aligned, centred on the entity's position, with half-extents of `ENTITY_SIZE * 0.5 * scale` per axis, where `ENTITY_SIZE` is a new engine constant giving the world-space size of an entity at scale 1 (the same size the editor renders and picks against, so pick and collision can't drift). The boxes are scale-boxes, not fitted to the mesh, so a sphere and a cube of equal scale collide identically. The sweep is O(n^2) over live entities. `World.collisions` is transient, derived state: it is recomputed on each run and skipped by serde, so it never saves with the scene.
...
- A right inspector dock with two tabs. The Scene tab lists every live entity and lets you select one by clicking it. The Inspector tab shows the selected entity's position and velocity as draggable number fields, a colour picker for its colour, draggable fields for its scale (x, y, z), a dropdown for its mesh primitive (cube, sphere, or plane), a checkbox for whether it is `Controlled` (WASD-driven), and a searchable picker that assigns one of the library's scripts to the entity. Every edit writes straight back into the world.
...
Runs entity scripts through a Rhai backend. The editor provides the engine's `ScriptRuntime` as a `rhai` interpreter — the one and only place `rhai` is used. Each tick it resolves an entity's script name to the library source, compiles it (caching by source text, so a script shared by many entities compiles once, and editing it recompiles once for all of them), hands the entity's current state to the script as variables, runs it, and writes the changes back. Scripts run before the movement systems each tick, so a script that sets a velocity has it applied the same tick. A named script that fails to compile is reported once and then skipped, so one bad script cannot stall the loop. As you edit, the Script Editor also compile-checks the open script and shows a status line — green when it parses, or the syntax error's line, column, and message when it doesn't. This is a syntax check only: because Rhai is dynamically typed, an unknown variable or a type mismatch is a run-time error, not a compile one, and won't show there.

A Script Editor for the library. The centre area tabs between the 3D Viewport and a Script Editor. The Script Editor is a two-pane split: a left sidebar listing every library script by name — where you add, select, and delete scripts — and a single large code editor for the selected one, with a line-number gutter. Because scripts are shared by name, this is where a behaviour is written once; assigning it to entities happens in the Inspector. The author-facing reference for what a script can do lives in SCRIPTING.md.
...
- Richer scripting in the editor: semantic checks beyond syntax (flagging a variable that isn't part of the script API, which Rhai only catches at run time), and a more discoverable script API surface than the current flat variable names (see Open questions).
