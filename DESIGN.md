...
Collision is detection-only and coarse. The `collision` system finds overlapping boxes and records the pairs, but nothing responds to them yet — no separation, no bounce, and scripts can't query them. The boxes are axis-aligned scale-boxes, not fitted to the actual mesh (a flat plane still has a tall box), and the sweep is O(n^2). Response, mesh-fitted or rotated boxes, exposing collisions to scripts, and a broad phase for large entity counts are all open, in roughly that order of likely usefulness.
...
Collision feedback. The editor runs the engine's `collision` system every frame and tints any overlapping entity red in the viewport, so an overlap is obvious while you arrange a scene — including while paused, since detection is recomputed from the current positions each frame. It is purely a visual flag; the entity's own colour in the world is unchanged. (Running it per frame rather than per tick is fine while nothing in the simulation consumes the result; it would move into the tick loop once collisions drive behaviour.)
